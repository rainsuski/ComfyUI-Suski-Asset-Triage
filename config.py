...
# 插件专用自闭环缓存目录: 放在插件项目自身目录下，杜绝污染 ComfyUI 根目录
...
# 2. 默认缩略图与画质规范
# ---------------------------------------------------------
DEFAULT_THUMB_MAX_EDGE = 512  # 缩略图长边默认限制 (px)
DEFAULT_THUMB_QUALITY = 80  # WebP 压缩质量 (1~100)
DEFAULT_THUMB_FORMAT = "WEBP"  # 缩略图标准格式
...
DEFAULT_PRESET = {
    ...
    "quality": 95,  # 仅 WEBP/JPEG 生效
    "path_template": "%date%/%category%",
    "name_template": "%date%_%model%_%seed%_%count%",
    "embed_workflow": True,
    "embed_prompt": True,
    "embed_lora": True,
}
